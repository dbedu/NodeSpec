...
function upload_results_to_api(){
    ...
    while IFS= read -r -d '' file; do
        ...
        for filename in "$header_info_filename" "$basic_info_filename" "$yabs_json_filename" \
                       "$ip_quality_filename" "$ip_quality_json_filename" "$net_quality_filename" \
                       "$net_quality_json_filename" "$backroute_trace_filename" \
                       "$backroute_trace_json_filename" "$port_filename"; do
            file_path="$result_directory/$filename"
            [[ -f "$file_path" ]] && printf '%s\0' "$file_path"
        done
    )
...
# ==================================================================
function generate_final_report_legacy(){
    ...
    local trace_log_chroot="/result/$backroute_trace_filename"
